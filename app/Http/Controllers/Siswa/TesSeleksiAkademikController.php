...
class TesSeleksiAkademikController extends Controller
{
    ...
    public function index(){
        ...
        if ($this->timeCheck() && ($step->step_4 == 'active' || $step->step_5 == 'active' || $step->step_6 == 'active')) {
            // var_dump(json_encode($this->user->calonsiswa->tesseleksi->status_tes_seleksi));dd();
            ...
            return view('pages.siswa.tesseleksi.index2', compact('jadwal_tes','message','route','message_pembayaran','message_pembayarans','calonsiswa'));    
        } else {
            return view('pages.siswa.pembayaran.index',compact('pembayaran','message_pembayarans','calonsiswa'));
        }
    }
        ...
        if ($this->user->calonsiswa->tesseleksi->status_tes_seleksi == 'selesai' || $this->user->calonsiswa->tesseleksi->status_tes_seleksi == 'sudah lewat') {
            ...
            if ($this->timeCheck()) {
                ...
                if ($no_pertanyaan < $total_pertanyaan) {
                    ...
                    return view('pages.siswa.tesseleksi.quiz', compact('quiz','no_pertanyaan','total_pertanyaan','countdown','calonsiswa'));
                } else {
                    ...
                    return view('pages.siswa.tesseleksi.quiz', compact('quiz_done','calonsiswa'));
                }
            } else {
                $quiz_timeout = "Maaf, Waktu Sudah Habis";
                return view('pages.siswa.tesseleksi.quiz',compact('quiz_timeout','calonsiswa'));
            }
        }
    ...
    public function hasil(){
        ...
        $tesseleksi = $this->user->calonsiswa->tesseleksi;
        // var_dump(json_encode($tesseleksi));dd();
        return view('pages.siswa.tesseleksi.hasil', compact('tesseleksi','calonsiswa','soal'));
    }

    public function detail(){
        $detailtes = $this->user->calonsiswa->tesseleksi->detail->get();
        $calonsiswa = $this->user->calonsiswa;
        return view('pages.siswa.tesseleksi.detail', compact('detailtes','calonsiswa'));
    }
