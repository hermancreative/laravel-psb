...
class SeleksiPenerimaanController extends Controller
{
    ...
    public function index(){
        ...
        $no_pendf_siswa_lulus = DB::table('tes_seleksis')
                            ...
                            //     })->get();
		$belum_diterima = "";
		// var_dump(json_encode($no_pendf_siswa_lulus));dd();
		if (count($no_pendf_siswa_lulus) > 0){
    		...
    		return view('pages.admin.seleksipenerimaan.index', compact('tahunajarans','jadwal','belum_diterima','sudah_diterima','backdatas','admin'));
		} else {
			$belum_diterima = [];
			$sudah_diterima = CalonSiswa::where('id_jadwal', $id_jadwal_terbuka)->where('status_penerimaan','diterima')->get();
			// var_dump(json_encode($sudah_diterima));dd();  
			$backdatas = [];
			return view('pages.admin.seleksipenerimaan.index', compact('tahunajarans','jadwal','belum_diterima','sudah_diterima','backdatas','admin'));
		}
